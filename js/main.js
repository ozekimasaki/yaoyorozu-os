...
async function startDesktop() {
  ...
  const kagiPill = document.getElementById("kagi-pill");
  if (kagiPill) {
    kagiPill.addEventListener("click", () => {
      if (!kernel.kagi) return;
      if (kernel.kagi.locked()) return;
      kernel.kagi.lock();
    });
  }
  const kagiOpen = document.getElementById("kagi-open");
  const kagiKotoba = document.getElementById("kagi-kotoba");
  const kagiErr = document.getElementById("kagi-err");
  if (kagiOpen && kagiKotoba) {
    const tryOpen = () => {
      if (!kernel.kagi) return;
      try {
        kernel.kagi.unlock(kagiKotoba.value);
        kagiKotoba.value = "";
        if (kagiErr) kagiErr.textContent = "";
      } catch (err) {
        if (kagiErr) kagiErr.textContent = err.message || "EPERM";
      }
    };
    kagiOpen.addEventListener("click", tryOpen);
    kagiKotoba.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        tryOpen();
      }
  ...
  const kagiKashiwa = document.getElementById("kagi-kashiwa");
  if (kagiKashiwa) {
    kagiKashiwa.addEventListener("click", () => kashiwa.open());
  }
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (kernel.kagi && kernel.kagi.locked()) {
      if (e.target && e.target.closest && e.target.closest("#kagi-veil")) return;
  ...
  eaves.addEventListener("click", (e) => {
    ...
    if (act === "kagi" && kernel.kagi) kernel.kagi.lock();
  });

export default {
  ...
  spawn({ kernel, launch }) {
    ...
    function bindOnce() {
      ...
      el.querySelector("#ping-to").addEventListener("keydown", (e) => {
        if (e.key === "Enter") {
          e.preventDefault();
          kernel.ping(el.querySelector("#ping-to").value.trim() || "縁");
        }
    ...
    const render = () => {
      bindOnce();
      const stat = `東京負荷 ${kernel.state.tokyo.toFixed(0)}% · 地方神 ${kernel.state.local.toFixed(0)}% · ソケット ${kernel.state.sockets.length}`;
      const statEl = el.querySelector("#net-stat");
      if (statEl && statEl.textContent !== stat) statEl.textContent = stat;
      const rows = (kernel.state.sockets || []).slice(0, 24);
      const sig = `${kernel.state.tokyo | 0}|${kernel.state.local | 0}|${rows.map((s) => `${s.id}:${s.ttl}:${s.state}`).join("|")}`;
